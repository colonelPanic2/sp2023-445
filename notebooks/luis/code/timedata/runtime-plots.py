...
def create_subplots(data,numtests_or_testidx,num_its_total,num_its_test=None,align='center',color=['red','black','red','black','red']):
    ...
    if 'num_its' in list(data.keys()):
        if "{}overall.png".format(pwd) in dirs:
            subprocess.run(['rm','{}overall.png'.format(pwd)])
        subprocess.run(["touch","{}overall.png".format(pwd)])
        ...
        if pwd+"test{}.png".format(numtests_or_testidx) in dirs:
            subprocess.run(['rm',pwd+'test{}.png'.format(numtests_or_testidx)])
        subprocess.run(["touch",pwd+"test{}.png".format(numtests_or_testidx)])
        num_its,runtimes=[],[]
        for state in states:
