...
While we were trying to solve the issues with the motors, we repurposed pins 3, 13, and 16 on the MCU. Pin 3 (PD1) on the MCU was always high, regardless of the input from the Pi. So we stopped using this pin and repurposed pin 13 (PD7) on the MCU, which was originally the Proximity_Data output from the microcontroller to the Pi, as the Rmotors_in1 output from the Pi to the microcontroller. We later repurposed pin 16 (PB2) on the MCU, which was originally the ECHO_1 output from the second ultrasonic sensor to the MCU, as the Proximity_Data output from the MCU to the Pi. This means that now only one ultrasonic sensor is connected to the MCU. We would also go on to remap the rest of the GPIO pins used on the Pi out of concern that some of the GPIO pins may have been broken, which may have been the explanation as to why pin 3 on the MCU was always high, but this had no effect. We also decided to add an ultrasonic sensor (HC-SR04) to the front and back of the car in order to detect obstacles and avoid collisions. The new pin mappings are shown in the chart below.
...
Taking the sum of these average runtimes and dividing by 5, we estimate our overall average runtime to be about 404.67 microseconds, far less than the anticipated runtime of no less than 50ms.

Overall, we can see from the data gathered today and in [2023-04-25](#2023-04-25---Demo-version-of-the-design) that the performance requirements regarding runtime for both the Pi code and the microcontroller code have been satisfied. This can be said for 2 reasons: First, the sum of the runtimes doesn't reach or exceed 500ms, and second, the Pi and MCU are running in parallel, meaning that the average runtime of an overall cycle for the design can be measured by the runtime of the Pi code alone as long as it is greater than the runtime of the MCU code.
